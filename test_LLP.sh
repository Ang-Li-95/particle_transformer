...
for i in "${fs[@]}"
do
  #weaver --predict --data-test "${DATADIR}/${i}_2018_test_1*.parquet" \
  weaver --predict --data-test "${DATADIR}/${i}_2018_test.parquet" \
   --data-config data/LLP/LLP_${FEATURE_TYPE}.yaml \
   --network-config $modelopts \
   --model-prefix ${MODEL_NAME} \
   --gpus "" --batch-size 128 \
   --num-workers 0 --fetch-step 1 \
   --predict-output ${i}_output.root
done

#weaver \
#    --data-train "${DATADIR}/stop*train.parquet" \
#    --data-test "${DATADIR}/stop*test.parquet" \
#    --data-config data/LLP/LLP_${FEATURE_TYPE}.yaml --network-config $modelopts \
#    --model-prefix training/LLP/${model}/{auto}${suffix}/net \
